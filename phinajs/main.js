...
const PLAYER_DEFAULT_SPEED = 4; //自機の移動スピード
const BULLET_DEFAULT_SPEED = 7; //自機が発射する弾のスピード
...
phina.define('EnemyGroup', {
    ...
    init: function (option) {
        ...
        this.moveRight = this.moveLeft + this.moveWidth;
    ...
    update: function (app) {
        ...
        // 左右の端についたら動く方向を変える
        if(this.x < this.moveLeft || this.x > this.moveRight){
            this.vx = -this.vx;
            this.y += 20;
        }
        // 動き
        this.x += this.vx;
        if(this.y > 280){
            alert('GAME OVER スコアは'+score+'点でした');
            app.stop();
            return;
        }
